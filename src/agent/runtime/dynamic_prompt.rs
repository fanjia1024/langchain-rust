    ...
    async fn before_agent_plan_with_runtime(
        ...
        if let Some(runtime) = runtime {
            ...
            if let Some(chat_history) = modified_input.get_mut("chat_history") {
                ...
                modified_input.insert("dynamic_system_prompt".to_string(), 
                    serde_json::json!(dynamic_prompt));
            } else {
                // Add as system prompt
                modified_input.insert("system_prompt".to_string(), 
                    serde_json::json!(dynamic_prompt));
            }
...
mod tests {
    ...
    async fn test_dynamic_prompt_middleware() {
        ...
        if let Ok(Some(modified)) = result {
            assert!(modified.contains_key("dynamic_system_prompt") || 
    ...
    async fn test_dynamic_prompt_middleware_no_runtime() {
        let middleware = DynamicPromptMiddleware::new(|_ctx: &dyn ToolContext| {
