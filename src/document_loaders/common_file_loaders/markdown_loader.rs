...
/// Markdown loader that loads Markdown files
#[derive(Debug, Clone)]
pub struct MarkdownLoader<R> {
    reader: R,
}

impl<R: Read> MarkdownLoader<R> {
    pub fn new(reader: R) -> Self {
        Self { reader }
    }
}

impl MarkdownLoader<Cursor<Vec<u8>>> {
...
#[async_trait]
impl<R: Read + Send + Sync + 'static> Loader for MarkdownLoader<R> {
    async fn load(
        mut self,
    ) -> Result<
...
mod tests {
    ...
    async fn test_markdown_loader() {
        ...
        let loader = MarkdownLoader::from_string(input);
        ...
        assert!(documents[0].page_content.contains("Title"));
