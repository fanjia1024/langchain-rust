...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    use langchain_ai_rust::vectorstore::{pgvector::PgOptions, VectorStore};
    let _ = store
    ...
    let llm_box1: Box<dyn langchain_ai_rust::language_models::llm::LLM> = Box::new(llm.clone());
    let llm_box2: Box<dyn langchain_ai_rust::language_models::llm::LLM> = Box::new(llm.clone());
