use langchain_rust::langgraph::{
    function_node_with_store, 
    persistence::{InMemorySaver, InMemoryStore, RunnableConfig}, 
    StateGraph, MessagesState, END, START,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let call_model = function_node_with_store(
        "call_model",
        |state: &MessagesState, config: &RunnableConfig, store: &dyn langchain_rust::langgraph::persistence::Store| async move {
            ...
            // Search for relevant memories based on last message
            let last_message = state.messages.last()
                .map(|m| m.content.as_str())
                .unwrap_or("");
            
            let memories = store.search(namespace, Some(last_message), Some(3)).await
            ...
            let response_text = if !memory_context.is_empty() {
                format!("Based on your memories: {}. Response to: {}", memory_context, last_message)
            } else {
    ...
    let compiled = graph.compile_with_persistence(Some(checkpointer.clone()), Some(store.clone()))?;
    ...
    let initial_state1 = MessagesState::with_messages(vec![
        Message::new_human_message("Hi! Remember: my name is Bob"),
    ]);
    
    let result1 = compiled.invoke_with_config(Some(initial_state1), &config1).await?;
    ...
    let initial_state2 = MessagesState::with_messages(vec![
        Message::new_human_message("What is my name?"),
    ]);
    
    let result2 = compiled.invoke_with_config(Some(initial_state2), &config2).await?;
