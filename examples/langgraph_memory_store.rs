...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let process_node = function_node_with_store(
        ...
        |state: &MessagesState, config: &RunnableConfig, store: &dyn langchain_rust::langgraph::persistence::Store| async move {
            ...
            let namespace = &["user_data", &user_id];
            ...
            let stored_pref = store.get(namespace, "preference").await
                .map_err(|e| LangGraphError::ExecutionError(format!("Store error: {}", e)))?;
            let pref_text = stored_pref
                .and_then(|item| item.value.get("value").and_then(|v| v.as_str()))
                .unwrap_or("no preference")
                .to_string();
            
            // Generate response
            let response = if pref_text != "no preference" {
                format!("I remember you like {}. You said: {}", pref_text, last_message)
            } else {
                format!("You said: {}. I'll remember your preferences.", last_message)
            };
    ...
    let config1 = {
        ...
        cfg.configurable.insert("user_id".to_string(), serde_json::json!("alice"));
        cfg
    };
    
    let state1 = MessagesState::with_messages(vec![
        Message::new_human_message("I like pizza"),
    ]);
    ...
    let state2 = MessagesState::with_messages(vec![
        Message::new_human_message("What do I like?"),
    ]);
